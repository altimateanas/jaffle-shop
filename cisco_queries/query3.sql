WITH MAXDT  AS (SELECT MAX(TO_DATE(RECORDEDAT)) AS MAX_SNAPSHOT_DT FROM cisco_demo.edw_telmtry_br_db_br.mt_cp_telmtry_dnac)
    ,MINDT  AS (SELECT COALESCE(MAX(SNAPSHOT_DATE),'1900-01-01') AS MAX_SNAPSHOT_DT FROM cisco_demo.edw_telmtry_br_db_br.mt_installed_prod_telmtry_dnac)
    ,MTDNAC AS (SELECT MEMBERID AS MEMBER_ID
                 ...
                 FROM cisco_demo.edw_telmtry_br_db_br.mt_cp_telmtry_dnac
				 WHERE TO_DATE(RECORDEDAT) > (SELECT MAX_SNAPSHOT_DT FROM MINDT )
		       ) --select * from MTDNAC;
,MT_CP_TELMTRY_THRDNG_MNGD_DEV as (select * from   cisco_demo.edw_telmtry_br_db_br.mt_cp_telmtry_thrdng_mngd_dev
                            where RECORDTYPE = 'CST_DNAC'
                            AND CREATED_BY_DEVICE > (SELECT MAX_SNAPSHOT_DT FROM MINDT )
                                ) --Change_log_7

,NCP_DATA_SS AS (SELECT * FROM cisco_demo.edw_telmtry_etl_db_ss.dnac_ncpdata )  --Change_log_5
...
INNER JOIN (SELECT DISTINCT TELEMETRY_ID,
									  ...
									  RULE_NAME ,
									  SRC_IDENTIFIER
								 FROM cisco_demo.edw_telmtry_br_db_br.n_installed_prod_telemetry LEAN
									  ...
									  SELECT  MEMBER_ID
										...
										INNER JOIN (SELECT
													...
													AND INV_PID IN (
																...
																FROM
                                                                cisco_demo.edw_ref_br_db_br.r_products
...
,ASSURANCE_DATA AS      (SELECT MEMBERID AS MEMBER_ID
						  ...
						  FROM (SELECT RECORDEDAT
									  ...
									  ,COUNT AS COUNTS
									  ,COLLECTEDON
							     FROM cisco_demo.edw_telmtry_etl_db_ss.dnac_assurancecounts A
...
,MT_NCP AS (SELECT A.PRIMARYKEY AS MEMBER_ID
	 ...
	 FROM cisco_demo.edw_telmtry_br_db_br.mt_cp_telmtry_dnac_ncp  A
...
FROM cisco_demo.edw_telmtry_etl_db_ss.dnac_base WHERE CALENDAR_DATE <= (SELECT MAX_SNAPSHOT_DT FROM MAXDT ) ) -- <= MAXDT max_SNAPSHOST_DATE
    ...
    FROM cisco_demo.edw_telmtry_etl_db_ss.dnac_clickeventapistat CEAPI
	JOIN FY_CALENDAR
...
,PRODUCT   AS (SELECT BK_PRODUCT_ID
                     ...
                     FROM cisco_demo.edw_ref_br_db_br.r_products A
...
,IOTBU_INV_AGG AS (SELECT MEMBER_ID
                    ...
                    (
                     ...
                     INNER JOIN (SELECT BK_PRODUCT_ID,LEVEL5_COMP_NAME from cisco_demo.edw_ref_br_db_br_view.mt_cx_solution_hierarchy
...
,IOTBU_SITE_AGG AS (SELECT MEMBER_ID
                    ...
                    FROM
                      ...
                      INNER JOIN (SELECT DISTINCT BK_PRODUCT_ID,LEVEL5_COMP_NAME from cisco_demo.edw_ref_br_db_br_view.mt_cx_solution_hierarchy WHERE LEVEL3_COMP_KEY in (14,15,16,17,18,130,131,132,133,147)) CX_SOL
